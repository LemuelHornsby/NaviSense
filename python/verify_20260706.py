#!/usr/bin/env python3
"""WP-20260706 verify — demo-week hold-the-line (T-5).
...
from the tools it reads. Writes Saved/NaviSense_Reports/wp_20260706_result.json.
Exit 0 = PASS, 1 = FAIL.
"""
...
def main():
    ...
    result = {
        ...
        "note": ("GO = headless tree rebuild-safe + storyline green on today's disk. "
                 "Does NOT confirm the in-engine demo; G_*_UE eye-checks remain."),
    }
    REPORTS.mkdir(parents=True, exist_ok=True)
    out = REPORTS / "wp_20260706_result.json"
    out.write_text(json.dumps(result, indent=2), encoding="utf-8")

    passed = sum(1 for c in checks if c["pass"])
    print(f"WP-20260706 verify -- {'PASS' if ok else 'FAIL'} {passed}/{len(checks)} gates")
    for c in checks:
